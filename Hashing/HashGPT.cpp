...
void insert_randomProbing(HashTable* ht, int key, int value) { // checar se nao esta cheio antes de inserir e se o elemento nao eh repetido
    if(ht->count >= ht->maxSize || find(ht, key)) return; // checar se nao esta cheio antes de inserir e se o elemento nao eh repetido

    int index = HashFunction(key);

    if(ht->entries[index].occupied) {
        int i = 0;
        int newIndex;
        do {
            i++;    
        ...
        } while(!ht->entries[newIndex].occupied);
        index = newIndex;
    }
